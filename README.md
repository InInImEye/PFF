...
- `results/`: Results generated by the simulation. Complete time evolution simulation given in `*.pvd`. Individual time-step simulation given in `*.pvtu`. Each processors solution for a particular timestep is present in `*.vtu`. The files `solution*` contains FEM node data and avg. of a few other internal data at the nodes. The files`tensor*` contain projections of the quadrature point data at the nodes.
- `CMakeLists.txt`: Instructions for properly compiling and linking files for the program.
- `pfc.cc`: Core of the program, contains the class PFC for simulating the phase field crack.
